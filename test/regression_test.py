...
"""Regression test for libopenspc.  A number of test files are loaded and
executed, and a checksum is performed on the output waveforms and compared
against an expected value.  This is meant purely to detect if the binary
output changes; there is otherwise no particular guarantee that the expected
values are 'correct'.  It is assumed that in the event of this test failing,
the test case will be reviewed manually, with the option of updating the
expected value if the new behavior truly is intended.
"""
...
TESTS = [
    ...
    ('env_timing.spc', '11e10a64915495d50f4eb4a6eaba6045'),
    # Test using direct envelope mode.
    ('direct_env.spc', '130a83c49b3af1d4930cf3cd4b4def4e'),
    # Test loading a ZSNES savestate.
    ('zsnes.zst', 'eadc717e84b29614ba397af3d71026af'),
]
...
def _visible_name(test_filename):
    return os.path.splitext(test_filename)[0]


def _data_filename(name):
    return os.path.join(this_dir, 'data', name + '.xz')
...
def run_test(name, output_dir, libpath):
    ...
    if output_dir is not None:
        out_file = open(
            os.path.join(output_dir, '%s.raw' % _visible_name(name)),
            'wb')
...
def main():
    ...
    if args.list:
        for test_no, (name, _) in selected_tests:
            print('%d: %s' % (test_no, _visible_name(name)))
        return
    ...
    for test_no, (name, expected_md5) in selected_tests:
        ...
        if args.verbose:
            print('%d (%s): %s' %
                  (test_no, _visible_name(name),
                   'OK' if ok else
                   ('FAILED (expected %r got %r)' %
                    (expected_md5, actual_md5))))
        if not ok:
            print('*** FAILED test %d (%s)' % (test_no, _visible_name(name)),
                  file=sys.stderr)
            failed = True
    if args.verbose:
        print()
